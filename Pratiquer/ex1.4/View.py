...
class View(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Création du bouton
        self.plot = QtWidgets.QPushButton('Info', self)#Push buttons display a textual label, and optionally a small icon. self -> the parent
...
#   definition de méthodes qui sont utilisées pour créer la table au-dessus
    def LineColours(self):
        colours = self.colours.currentText()
        return str(colours)
    
    def GridLines(self):
        lines = self.grid_lines.checkState() == QtCore.Qt.Checked
        return str(lines)
    
    
    def FrequencyValue(self):
        FreqVal = self.freq.text()
        return str(FreqVal)
    
    def PhaseValue(self):
        PhaValue= self.phi.text()
        return str(PhaValue) 
